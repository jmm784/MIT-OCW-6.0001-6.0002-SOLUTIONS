...
class RectangularRoom(object):
    ...
    def __init__(self, width, height, dirt_amount):
        ...
        """
        #Note: remember to convert positino floats
        
        assert type(width) == int and type(height) == int and type(dirt_amount) == int
        assert width > 0 and height > 0 and dirt_amount >= 0
        
        self.width = width
        self.height = height
        self.dirt_amount = dirt_amount
        self.cleaned_tiles = {}
        self.cleaned_tiles_keys = list(self.cleaned_tiles.keys())
    ...
    def clean_tile_at_position(self, pos, capacity):
        ...
        """
        #Note: use the self.cleaned_tiles initialisation
        
        tile = tuple([math.floor(pos.get_x), math.floor(pos.get_y)])
        
        if tile not in self.cleaned_tiles_keys:
            if self.dirt_amount >= capacity:
                self.cleaned_tiles[tile] = 0
            else:
                self.cleaned_tiles[tile] = self.dirt_amount - capacity
        
        else:
            if self.cleaned_tiles[tile] <= capacity:
                self.cleaned_tiles[tile] = 0
            else:
                self.cleaned_tiles[tile] = self.cleaned_tiles[tile] - capacity
    ...
    def is_tile_cleaned(self, m, n):
        ...
        Note: The tile is considered clean only when the amount of dirt on this
              tile is 0.
        """
        check_tile = tuple([m,n])
        
        if check_tile in self.cleaned_tiles_keys:
            if self.cleaned_tiles[check_tile] == 0:
                return True
    ...
    def get_num_cleaned_tiles(self):
        ...
        """
        i = 0
        for key in self.cleaned_tiles_keys:
            if self.cleaned_tiles[key] == 0:
                i += 1
        return i
    ...
    def get_dirt_amount(self, m, n):
        ...
        """
        d_tile = tuple([m,n])
        return self.cleaned_tiles[d_tile]
