# -*- coding: utf-8 -*-
"""
Created on Sun Jul 28 14:06:01 2024
...
def get_best_path(digraph, start, end, path, max_dist_outdoors, best_dist,
    ...
    path[0] = path[0] + [start]
    path_dist = get_path_length(digraph, path[0])
    path[1] = path_dist[0]
    path[2] = path_dist[1]
    
    best_dist = get_path_length(digraph, best_path)[0]
    
    if best_dist == 0 or (path[1] < best_dist and path[2] < max_dist_outdoors):
        
        if start == end:
            tuple_path = tuple(path[0])
            return tuple_path
        
        for edge in digraph.get_edges_for_node(start_node):
            dest_node = edge.get_destination().get_name()
            if dest_node not in path[0]:
...
def directed_dfs(digraph, start, end, max_total_dist, max_dist_outdoors):
    ...
    """
    #the phrasing of the last part of this doctsring makes me think 
