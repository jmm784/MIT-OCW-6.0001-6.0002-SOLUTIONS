...
def get_word_score(word, n):
    ...
    C1 = 0
    for i in word:
        C1 += SCRABBLE_LETTER_VALUES[i]
    
    # Second component
    C2 = 7*word_length - 3*(n-word_length)
    if 1 > C2:
        C2 = 1
    return C2*C1
...
def update_hand(hand, word):
    ...
    word = word.lower()
    for letter in word:
        if letter in hand_new.keys() and hand_new[letter] > 1:
            hand_new[letter] = hand_new[letter] - 1
        elif letter in hand_new.keys():
            del hand_new[letter]
    return hand_new
...
def is_valid_word(word, hand, word_list):
    ...
    if word in word_list:
        for letter in word:
            if letter not in hand.keys() or word.count(letter) > hand.get(letter, 0):
                return False
        return True
    
    elif "*" in word:
        for letter1 in VOWELS:
            if word.replace("*", letter1) in word_list:
                for letter2 in word:
                    if letter2 not in hand.keys() or word.count(letter2) > hand.get(letter2, 0):
                        return False
                return True
...
def play_hand(hand, word_list):
    ...
    * The hand finishes when there are no more unused letters.
    ...
    while calculate_handlen(hand) > 0:  # As long as there are still letters left in the hand:
        ...
        if word == "!!":  # If the input is two exclamation points:
            ...
            if is_valid_word(word, hand, word_list):  # If the word is valid:
                ...
                print('"' + word + '"', "earned", get_word_score(word, n), "points. Total:", score, "points")  # Tell the user how many points the word earned,
    ...
    return score  # Return the total score as result of function
...
def play_game(word_list):
    ...
    * Returns the total score for the series of hands
    ...
    for i in range(num_hands):
        ...
        # maybe lowercase sub_letter here?
        if sub_letter == 'yes':
            sub_letter_yes = input("Which letter would you like to replace: ")
            print()
            hand = substitute_hand(hand, sub_letter_yes)
        else:
            print()
        hand_score = play_hand(hand, word_list)
        print("----------")
        if rep_counter == 0:
            rep_hand = input("Would you like to replay the hand? ")
            if rep_hand == 'yes':
                rep_counter += 1
                hand_score = play_hand(hand, word_list)
